...
def createXcs(c):
	...
	with open(filename, "w", encoding="utf-8") as f:  # OptionsDialog.xcuファイルの作成
		...
		root[-1].append(Elem("group", {"oor:name": "Sizes"}))
		...
		root[-1][-1].append(Elem("group", {"oor:name": "Defaults"}))		
		root[-1][-1][-1].append(Elem("prop", {"oor:name": "Width", "oor:type": "xs:string"}))
		root[-1][-1][-1][-1].append(Elem("value", text="300"))
		root[-1][-1][-1].append(Elem("prop", {"oor:name": "Height", "oor:type": "xs:string"}))
		root[-1][-1][-1][-1].append(Elem("value", text="300"))		
		root.append(Elem("component"))
		root[-1].append(Elem("group", {"oor:name": "Leaves"}))
		root[-1][-1].append(Elem("node-ref", {"oor:name": "MaximumPaperSize", "oor:node-type": "Sizes"}))
